...
@Component({
  ...
  template: ` <div class="relative w-full min-h-12 bg-white px-3 py-2">
      ...
      >
        ...
        >
          <div class="flex items-center">
            ...
            @if(contact.avatar) {
            <div
              class="w-8 h-8 rounded-full bg-gray-300 mr-3 flex-shrink-0 overflow-hidden"
            >
              <img
                [src]="contact.avatar"
                [alt]="contact.name"
    ...
    @if(lastAddedContact()) {
    <div
      class="mt-2 alert alert-warning alert-soft flex items-center justify-between"
    >
      <span>
...
export default class Contacts {
  ...
  filteredContacts = computed(() => {
    ...
    return contacts.filter(
      (contact) =>
        !this.selectedContacts().some((selected) => selected.id === contact.id)
    );
  });
  ...
  removeContact(contact: SelectedContact): void {
    this.selectedContacts.update((prev) =>
  ...
  removeLastContact(): void {
    if (this.inputValue() === '' && this.selectedContacts().length > 0) {
      this.lastAddedContact.set(
        this.selectedContacts()[this.selectedContacts().length - 1]
      );
