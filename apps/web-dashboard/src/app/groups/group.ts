        ...
        <div class="card card-border border-base-300 bg-base-100">
          <div class="card-body flex flex-row justify-between items-center">
            <div>
              ...
              <h3 class="text-base-200">{{ group.studyPlan?.name }} / {{ group.studyPlan?.degree?.name }}</h3>
              <div class="flex items-center gap-2">
                <div class="avatar avatar-placeholder">
                  <div class="text-white w-7 rounded-full" [style.background]="group.teacher?.color">
            ...
            @if (canStartGroupChat()) {
              <button class="btn btn-ghost btn-sm" (click)="startGroupChat()" [disabled]="startingChat()">
                @if (startingChat()) {
