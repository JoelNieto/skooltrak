...
Next.js detects missing `@types/react` during `next build` and tries to install it using `yarn add` regardless of the actual package manager. In a Bun workspace, this fails with a "nearest package directory isn't part of the project" error.

**Root cause**: `@types/react` is missing from root devDependencies.
**Fix**: Install deps at the root before building: `bun add -d @types/react @types/react-dom`
...
### `@nx/next:init` Rewrites All package.json Scripts (Whole-Repo Import)
...
This is the standard "package.json Script Rewriting" issue from SKILL.md, but triggered by `@nx/next:init` rather than Nx init. **Fix**: Remove all rewritten scripts from `package.json` — `@nx/next/plugin` infers all targets from `next.config.*`.
...
3. Install Next.js-specific deps: `bun add -d @next/eslint-plugin-next`
...
- Errors found & fixed:
  1. package.json workspaces field: `apps`/`libs` → `apps/*`/`libs/*`
...
- Errors found & fixed:
  1. package.json workspaces field: `apps/web` → `apps/*`
  2. Stale files: `node_modules/`, `bun.lock from source`, `package.json workspaces field`, `.gitignore` — deleted
  3. Nx-rewritten package.json scripts (`"build": "nx next:build"`, etc.) — removed
...
- Next.js-specific findings:
  ...
  2. Stale files: `node_modules/`, foreign lockfile, `.gitignore` — deleted
...
- Errors found & fixed:
  ...
  2. Stale files from Vite source: `node_modules/`, `bun.lock from source`, `package.json workspaces field`, `.gitignore`, `nx.json`
  3. Removed rewritten scripts from Vite app's `package.json`
  4. ESLint 8 vs 9 conflict — `@nx/eslint` peer on ESLint 8 resolved wrong version. Fixed with `overrides`
